...
export function apply(ctx, rawConfig) {
  ...
  ctx.logger?.info?.(
    `dsh-answer-reviewer: mounted (threshold=${initialConfig.threshold}, maxChallenges=${initialConfig.maxChallenges}, config=${configPath ?? '(in-memory only)'}, server=http://127.0.0.1:${DEFAULT_HTTP_PORT})`,
  )
}
...
export async function onTurnStopping(ctx, store, counter, payload) {
  ...
  if (used >= config.maxChallenges) {
    store.recordActivity({ sessionId, turn, decision: 'cap-exhausted' })
    return
  }
  ...
  if (!score) {
    ...
    store.recordActivity({ sessionId, turn, decision: 'parse-fail' })
    return
  }
