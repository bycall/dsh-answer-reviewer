...
export { Config, resolveConfig, extractAssistantText, parseVerdict, buildSteerMessage, buildReviewPrompt, createChallengeCounter } from './review.js'
...
export function apply(ctx, rawConfig) {
  ...
  ctx.on('agent/turn-stopping', async (payload) => {
    await onTurnStopping(ctx, config, counter, payload).catch((error) => {
      // Defensive: a throwing review must NEVER block the agent turn.
      ctx.logger?.warn?.(`dsh-answer-reviewer: review crashed, allowing turn to close: ${String(error)}`)
    })
  })
  ctx.logger?.info?.(`dsh-answer-reviewer: mounted (maxChallenges=${config.maxChallenges})`)
}
...
export async function onTurnStopping(ctx, config, counter, payload) {
  ...
  // Counter is per (session, turn). If we've already exhausted the budget
  // for this turn, do nothing and let the turn close.
  const used = counter.get(sessionId, turn)
  if (used >= config.maxChallenges) return

  // Pull the assistant text for this turn.
  ...
  const prompt = buildReviewPrompt(assistantText)
  ...
  const verdict = parseVerdict(rawReply)
  if (!verdict) {
  ...
  // Fail path: bump the counter, steer, and let the machine re-read its inbox.
  const newCount = counter.bump(sessionId, turn)
  const message = buildSteerMessage(verdict, newCount, config.maxChallenges)
  try {
    agent.steer(message)
    ctx.logger?.info?.(`dsh-answer-reviewer: steered session=${sessionId} turn=${turn} (${newCount}/${config.maxChallenges})`)
  } catch (error) {
